...
OncoCast <- function(data,formula, method = c("ENET"),
  ...
  if("ENET" %in% method) {
    ...
    ENET <- foreach(run=1:runs) %dopar% {
      ...
      lambda1s <- c(0,0.1,0.5,1,5) #10
      lambda2s <- c(0,0.1,0.5,1,5)
  ...
  if("RF" %in% method){
    ...
    RF <- foreach(run=1:runs) %dopar% {
      ...
      rf <- try(randomForest(y~.,data = train,mtry = round(ncol(train)/3,digits = 0),importance=T,ntree = nTree),silent=T)
